...
export default function ForgotPassword() {
  ...
  const requestOtp = async (e) => {
    ...
    try {
      const res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'requestPasswordReset', email }) });
      const result = await res.json();
      closeAlert();
      if(result.status === 'success') {
        setStep(2);
        showSuccess('ส่ง OTP แล้ว', 'กรุณาตรวจสอบรหัส OTP ในอีเมลของคุณ');
      } else { showError('ไม่พบข้อมูล', result.message); }
  ...
  const resetPass = async (e) => {
    ...
    try {
      const res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'resetPassword', email, otp, newPassword }) });
      const result = await res.json();
      closeAlert();
      if(result.status === 'success') {
        await Swal.fire({
        ...
        {step === 1 ? (
          ...
          <form onSubmit={resetPass} className="space-y-4 relative z-10 animate-fade-in-up">
            ...
            <div>
              ...
              <input type="text" required className="bg-slate-900 border border-slate-600 rounded-lg w-full p-3 text-white text-center text-xl tracking-[0.5em] font-bold mt-1 focus:border-blue-500 outline-none" maxLength={6} value={otp} onChange={e => setOtp(e.target.value)} />
            </div>
            <div>
